...
## 0. 요약
Q1. 에이전트 수(N)가 늘어날 때 고정된 epsilon값이 유효한가?
- A : No
- 실험 결과 : epsilon=1.5일 때 N=3,5,7인 경우 모두 J값은 0
- 한계점 : gpt-4o-mini

Q2. 에이전트 수(N)가 커질수록 epsilon값도 커져야 하는가?
- A : No
- 실험 결과 : N이 커질수록 gap이 커짐(=에이전트 간 불일치 정도가 커짐). 즉 N이 커질수록 epsilon이 제 역할을 하지 못할 가능성이 큼.
- 한계점 : 표본이 적음(각 케이스 별 질문 3개에 대한 답변만으로 실험 진행)
...
- recall: 실제로 공격이 있었던 경우 중, BPD가 진짜 악의적 에이전트를 정확히 찾아낸 비율
- fpr: 공격이 없었던 경우 중에도 BPD가 누군가를 잘못 악의적 에이전트로 지목한 비율
- Youden's J: recall에서 fpr을 뺀 값으로, 공격자는 잘 잡으면서 오탐은 적은 epsilon일수록 높게 나오는 종합 지표
...
- gap: 한 에이전트의 점수가 다른 모든 에이전트들의 점수와 평균적으로 얼마나 차이 나는지를 나타내는 값
- malicious gap: 실제 공격이 있었던 시나리오에서, 진짜 악의적 에이전트의 gap
- benign gap: 같은 시나리오에서, 나머지 정상 에이전트들의 gap
- **어떤 epsilon 값을 골라도 N의 값이 커질수록 Youden's J값 자체가 낮을 수밖에 없는 이유**
